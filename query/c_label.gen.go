...
func newLabel(db *gorm.DB) label {
	...
	_label.labelDo.UseDB(db)
	_label.labelDo.UseModel(&model.Label{})

	tableName := _label.labelDo.TableName()
	_label.ALL = field.NewField(tableName, "*")
	_label.ID = field.NewInt32(tableName, "id")
...
func (l *label) WithContext(ctx context.Context) *labelDo { return l.labelDo.WithContext(ctx) }
